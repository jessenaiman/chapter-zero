    ...
    public partial class Scene1LoadTests : GodotObject
    {
        ...
        public void Scene1_AllThreadDataFilesHaveValidStructure()
        {
            ...
            foreach (string thread in threads)
            {
                ...
                // Assert
                Assert.IsTrue(doc.RootElement.TryGetProperty("type", out var typeProperty), 
                ...
                Assert.IsTrue(doc.RootElement.TryGetProperty("openingLines", out var openingLines), 
                    $"{thread}.json should have 'openingLines' array");
                Assert.IsTrue(openingLines.GetArrayLength() > 0, 
                    $"{thread}.json should have at least one opening line");
                
                Assert.IsTrue(doc.RootElement.TryGetProperty("initialChoice", out var initialChoice), 
                    $"{thread}.json should have 'initialChoice' object");
                Assert.IsTrue(initialChoice.TryGetProperty("prompt", out _), 
                    $"{thread}.json initialChoice should have 'prompt'");
                Assert.IsTrue(initialChoice.TryGetProperty("options", out var options), 
                    $"{thread}.json initialChoice should have 'options' array");
                Assert.IsTrue(options.GetArrayLength() > 0, 
