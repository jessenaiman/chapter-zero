...
func load_game_scene() -> void:
	OmegaSpiralGameState.start_game()
	super.load_game_scene()

func new_game() -> void:
	if confirm_new_game and OmegaSpiralGameState.get_levels_reached() > 0:
		new_game_confirmation.show()
	else:
		OmegaSpiralGameState.reset()
		load_game_scene()
...
func _show_level_select_if_set() -> void:
	...
	if OmegaSpiralGameState.get_levels_reached() <= 1: return
	level_select_button.show()

func _show_continue_if_set() -> void:
	if OmegaSpiralGameState.get_current_level_path().is_empty(): return
...
func _on_continue_game_button_pressed() -> void:
	OmegaSpiralGameState.continue_game()
	load_game_scene()
