...
public partial class GameState : Node
{
    ...
    public override async void _Ready()
    {
        ...
        // Allow scene tree to fully initialize before database operations
        ...
        try
        {
            ...
            // Use async migration to prevent blocking
            await context.Database.MigrateAsync().ConfigureAwait(false);

            _SaveLoadManager = new SaveLoadManager(context);
            GD.Print("[GameState] Database initialized successfully.");
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[GameState] Failed to initialize database: {ex.Message}");
            GD.PrintErr($"[GameState] Stack trace: {ex.StackTrace}");
