...
public partial class OpeningCutscene : OmegaSpiral.Source.Scripts.Field.cutscenes.Cutscene
{
    ...
    {
        await this.ShowBackgroundAsync().ConfigureAwait(false);
        await this.PlayDialogueAsync().ConfigureAwait(false);
        await this.PlayTransitionAsync().ConfigureAwait(false);
        this.HideBackground();
        this.StartMusic();
        await this.ClearTransitionAsync().ConfigureAwait(false);
