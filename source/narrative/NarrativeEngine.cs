// <copyright file="NarrativeEngine.cs" company="Ωmega Spiral">
...
{
    /// <summary>
    /// Generic narrative engine that drives a <see cref="NarrativeScript"/> using an <see cref="INarrativeHandler"/>.
    ...
    public sealed class NarrativeEngine
    {
        ...
        /// <param name="handler">UI‑side implementation of <see cref="INarrativeHandler"/>.</param>
        public async Task PlayAsync(NarrativeScript script, INarrativeHandler handler)
        {
            ...
            var scenes = script.Scenes ?? new List<NarrativeScene>();
        ...
        private async Task PlaySceneAsync(NarrativeScene scene, INarrativeHandler handler)
        {
            ...
            if (scene.Lines != null)
            {
                foreach (var line in scene.Lines)
                {
                    ...
                    if (line.StartsWith("[") && line.EndsWith("]"))
                    {
                        ...
                        if (!handled) displayLines.Add(line);
                    }
                    else
                    {
            ...
            if (!string.IsNullOrWhiteSpace(scene.Question) && scene.Answers?.Count > 0)
            {
                var selected = await handler.PresentChoiceAsync(
                    scene.Question,
                    scene.Speaker ?? scene.Owner ?? "system",
                    scene.Answers).ConfigureAwait(false);
