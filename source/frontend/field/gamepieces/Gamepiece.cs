...
public partial class Gamepiece : Node2D
{
    ...
    /// <summary>
    /// Gets the path the gamepiece is following as a read-only collection.
    /// </summary>
    private readonly List<Vector2I> _movePath = new();
    public ReadOnlyCollection<Vector2I> MovePath => _movePath.AsReadOnly();
    ...
    {
        if (this._movePath.Count == 0)
        {
            return;
        }
        // Get the next waypoint
        var nextWaypoint = this._movePath[0];
    ...
    public void SetMovePath(ReadOnlyCollection<Vector2I> path)
    {
        ...
        if (path != null)
        {
            ...
            {
                this._movePath.Add(cell);
            }
        }
        // If we're not already moving, start moving to the first waypoint
        if (!this.IsMoving && this._movePath.Count > 0)
        {
            this.MoveToNextWaypoint();
        }
    }
    ...
    /// </summary>
    /// <returns></returns>
    public ReadOnlyCollection<Vector2I> GetMoveableAdjacentCells()
    {
        var gameboard = this.GetNodeOrNull<global::OmegaSpiral.Source.Scripts.Field.gameboard.Gameboard>("/root/Gameboard");
        var adjacentCells = gameboard?.GetAdjacentCells(this.CellPosition) ?? new ReadOnlyCollection<Vector2I>(new List<Vector2I>());
        ...
        return moveableCells.AsReadOnly();
    }
