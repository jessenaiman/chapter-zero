...
public partial class GameManager : Node
{
    ...
    /// </summary>
    public IReadOnlyDictionary<string, int> DreamweaverPoints => this._DreamweaverPoints;
    ...
    private readonly Dictionary<string, int> _DreamweaverPoints = new(StringComparer.OrdinalIgnoreCase);
    ...
    private void AddDreamweaverPoints(string dreamweaverId, int points)
    {
        ...
        if (this._DreamweaverPoints.TryGetValue(dreamweaverId, out int current))
        {
            this._DreamweaverPoints[dreamweaverId] = current + points;
        }
        else
        {
            this._DreamweaverPoints[dreamweaverId] = points;
        }
    }

    private void LogDreamweaverPoints(string stageId)
    {
        if (this._DreamweaverPoints.Count == 0)
        {
            GD.Print($"[GameManager] No dreamweaver points recorded after {stageId}.");
            return;
        }

        var summary = string.Join(", ", this._DreamweaverPoints.Select(kvp => $"{kvp.Key}:{kvp.Value}"));
