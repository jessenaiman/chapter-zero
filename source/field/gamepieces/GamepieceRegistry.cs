...
public partial class GamepieceRegistry : Node
{
    ...
    public void UnregisterGamepiece(Gamepiece gamepiece)
    {
        ...
        if (!this.gamepieceToCell.TryGetValue(gamepiece, out var oldCell))
        {
            return;
        }
    ...
    public IReadOnlyCollection<Gamepiece> GetAllGamepieces()
    {
    ...
    public IReadOnlyCollection<Vector2I> GetAllOccupiedCells()
    {
    ...
    public IReadOnlyCollection<Gamepiece> GetGamepiecesInArea(Rect2I area)
    {
    ...
    public bool IsPathBlocked(IReadOnlyCollection<Vector2I> path)
    {
        if (path == null || path.Count == 0)
        {
            return false;
        }
        var pathList = path as List<Vector2I> ?? new List<Vector2I>(path);
        for (int i = 1; i < pathList.Count; i++)
        {
            if (this.IsCellOccupied(pathList[i]))
            {
                return true;
            }
        }
    ...
    public IReadOnlyCollection<T> GetGamepiecesOfType<T>()
        where T : Gamepiece
    {
