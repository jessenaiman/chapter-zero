...
public partial class Pathfinder : RefCounted
{
    ...
    public IReadOnlyCollection<Vector2I> GetPathToCell(Vector2I startCell, Vector2I endCell)
    {
    ...
    private bool TryGetPoints(int startId, int endId, out PointData? startPoint, out PointData? endPoint)
    {
    var foundStart = this.points.TryGetValue(startId, out startPoint);
    var foundEnd = this.points.TryGetValue(endId, out endPoint);
    if (!foundStart) startPoint = null;
    if (!foundEnd) endPoint = null;
    return foundStart && foundEnd;
    }

    private void InitializeStartPoint(PointData startPoint, PointData endPoint)
    {
        startPoint.GScore = 0;
        startPoint.FScore = HeuristicCostEstimate(startPoint.Position, endPoint.Position);
    }

    private bool ProcessNeighbors(int currentId, PointData endPoint, Dictionary<int, int> cameFrom, PriorityQueue<int, float> openSet, HashSet<int> openSetHash)
    {
        ...
        foreach (var neighborId in this.connections.GetValueOrDefault(currentId, new List<int>()))
        {
            ...
            var tentativeGScore = currentPoint.GScore + 1;
    ...
    public IReadOnlyCollection<Vector2I> GetPathCellsToAdjacentCell(Vector2I sourceCell, Vector2I targetCell)
    {
        // Find adjacent cells to the target
        var adjacentCells = this.GetAdjacentCells(targetCell);
            IReadOnlyCollection<Vector2I> shortestPath = new List<Vector2I>();
