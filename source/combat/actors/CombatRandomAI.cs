...
public partial class CombatRandomAI : CombatAI
{
    ...
    public override async Task<(BattlerAction? action, List<Battler> targets)> ChooseAction()
    {
        ...
        var targets = await this.SelectTargetsForAction(chosenAction).ConfigureAwait(false);
    ...
    public override float GetActionPriority(BattlerAction action, ReadOnlyCollection<Battler> targets)
    {
        return 1.0f;
    }
}
