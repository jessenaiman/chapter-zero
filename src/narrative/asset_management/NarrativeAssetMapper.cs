...
public partial class NarrativeAssetMapper : Node
{
    ...
    /// <returns>A collection of asset names of the specified type.</returns>
    public Collection<string> GetAssetsByType(AssetType type)
    {
        if (this.assetTypeMap.TryGetValue(type, out List<string>? assets))
        {
            return new Collection<string>(assets);
        }
    ...
    /// </summary>
    /// <returns>A collection of all asset names.</returns>
    public Collection<string> GetAllAssetNames()
    {
        return new Collection<string>(new List<string>(this.assetMap.Keys));
    }
