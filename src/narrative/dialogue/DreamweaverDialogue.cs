...
public class DreamweaverDialogueData : BaseDialogueData
{
    ...
    public DreamweaverDialogueData(string dreamweaverId, DreamweaverType type,
                          IReadOnlyList<string> openingLines, IReadOnlyList<string> dialogueLines,
                          IReadOnlyList<IDialogueChoice> choices, IReadOnlyList<INarrativeBlock> narrativeBlocks)
...
public partial class DreamweaverDialogueManager : BaseDialogueManager
{
    ...
    private DreamweaverDialogueData CreateFallbackDreamweaverDialogue(string dwId)
    {
        return new DreamweaverDialogueData(
            ...
            new ReadOnlyCollection<string>(new List<string> { "A familiar voice whispers...", "The echo calls to you." }),
            new ReadOnlyCollection<string>(new List<string> { "You feel a presence near you." }),
            new ReadOnlyCollection<IDialogueChoice>(new List<IDialogueChoice>()),
            new ReadOnlyCollection<INarrativeBlock>(new List<INarrativeBlock>())
        )
        {
            IsMajorBeat = false,
            Priority = 0,
