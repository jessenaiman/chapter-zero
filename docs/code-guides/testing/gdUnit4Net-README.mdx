...
GdUnit4Net supports advanced test filtering using VSTest syntax:
...
dotnet test --filter "(TestCategory=UnitTest)&(Trait.Owner=TeamA)"
```

Example test with category and trait:
...
<p align="center"><a href="https://github.com/MikeSchulze/gdUnit4Net"><img src="https://github.com/MikeSchulze/gdUnit4Net/blob/master/Example/assets/TestExplorerRun.png" width="100%"/></a><br/></p>
...
- All code style and analyzer rules are now managed in `.editorconfig` at the solution root.
- Only Roslynator analyzers are enforced; StyleCop and ruleset files are no longer used.
- Remove any `stylecop.json`, `.ruleset`, or related configuration files from the project.
