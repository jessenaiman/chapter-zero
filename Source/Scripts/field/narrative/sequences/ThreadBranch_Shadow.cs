// <copyright file="ThreadBranch_Shadow.cs" company="Ωmega Spiral">
...
/// </summary>
[GlobalClass]
public partial class ThreadBranchShadow : NarrativeSequence
{
    /// <summary>
    /// Scene data loaded from the Shadow thread configuration.
    /// </summary>
    ...
    protected override void OnSequenceReady()
    {
        ...
        // Load Shadow thread data
        this.LoadShadowSceneData();
    }
    ...
    public override async Task PlayAsync()
    {
        ...
        {
            GD.PrintErr($"[{this.SequenceId}] Failed to load Shadow scene data");
    ...
    /// <summary>
    /// Loads the Shadow thread scene data from configuration.
    /// </summary>
    private void LoadShadowSceneData()
    {
        try
        {
            string path = "res://Source/Data/stages/ghost-terminal/shadow.yaml";
            ...
            if (this.sceneData != null)
            {
                GD.Print($"[{this.SequenceId}] Loaded Shadow scene data with {this.sceneData.StoryBlocks.Count} story blocks");
            }
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[{this.SequenceId}] Exception loading Shadow scene data: {ex.Message}");
        }
    }

    /// <summary>
    /// Displays the opening lines for the Shadow thread.
    /// </summary>
    ...
    /// <summary>
    /// Displays the exit line to conclude the Shadow thread.
    /// </summary>
