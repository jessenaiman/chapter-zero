// <copyright file="ThreadBranch_Hero.cs" company="Ωmega Spiral">
...
/// <summary>
/// Thread branch sequence for Hero-aligned narrative continuation.
/// Presents Hero-specific story content after the opening choice.
/// </summary>
[GlobalClass]
public partial class ThreadBranchHero : NarrativeSequence
{
    /// <summary>
    /// Scene data loaded from the Hero thread configuration.
    /// </summary>
    ...
    protected override void OnSequenceReady()
    {
        ...
        // Load Hero thread data
        this.LoadHeroSceneData();
    }
    ...
    public override async Task PlayAsync()
    {
        ...
        {
            GD.PrintErr($"[{this.SequenceId}] Failed to load Hero scene data");
    ...
    /// <summary>
    /// Loads the Hero thread scene data from configuration.
    /// </summary>
    private void LoadHeroSceneData()
    {
        try
        {
            string path = "res://Source/Data/stages/ghost-terminal/hero.yaml";
            ...
            if (this.sceneData != null)
            {
                GD.Print($"[{this.SequenceId}] Loaded Hero scene data with {this.sceneData.StoryBlocks.Count} story blocks");
            }
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[{this.SequenceId}] Exception loading Hero scene data: {ex.Message}");
        }
    }

    /// <summary>
    /// Displays the opening lines for the Hero thread.
    /// </summary>
    ...
    /// <summary>
    /// Displays the exit line to conclude the Hero thread.
    /// </summary>
