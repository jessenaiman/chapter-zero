...
public partial class OpeningSequence : NarrativeSequence
{
    ...
    /// <summary>
    /// Terminal UI component for enhanced narrative presentation.
    /// </summary>
    ...
    private async Task DisplayTextWithEnhancedTypewriterAsync(string text)
    {
        ...
        {
            // Use TerminalUI's enhanced typewriter with effects
            await this.terminalUI.DisplayTextWithTypewriterAsync(text).ConfigureAwait(false);
        }
        else if (this.Dialogue != null)
        {
            // Fallback to UIDialogue
    ...
    private async Task PresentThreadChoiceAsync()
    {
        ...
        // Display the choice prompt in terminal
        await this.DisplayTextWithEnhancedTypewriterAsync(this.sceneData.InitialChoice.Prompt!).ConfigureAwait(false);
    ...
    private void UpdateChoiceButtonTexts()
    {
        ...
        this.heroButton!.Text = OpeningSequence.GetButtonText(options[0]);
        this.shadowButton!.Text = OpeningSequence.GetButtonText(options[1]);
        this.ambitionButton!.Text = OpeningSequence.GetButtonText(options[2]);
    }
    ...
    private static string GetButtonText(DreamweaverChoice option)
    {
