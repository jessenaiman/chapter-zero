...
public partial class Stage4Main : Node2D
{
    ...
    private Dictionary<string, Node2D> DreamweaverPresences = new();
    ...
    private async Task ShowInitialNarrative()
    {
        ...
        var dialogue = await _dialogueManager.GetNpcDialogueAsync("narrator").ConfigureAwait(false);
    ...
    {
        var dialogueData = await _dialogueManager.GetNpcDialogueAsync(npcId).ConfigureAwait(false);
    ...
    public async void InteractWithNpc(string npcId)
    {
        ...
        await _dialogueManager.StartDialogueAsync(npcId).ConfigureAwait(false);
    }
}
