...
/// Inherits from <see cref="ManifestAwareNode{T}"/> to provide unified manifest loading.
/// </summary>
[GlobalClass]
public abstract partial class StageController : ManifestAwareNode<StageManifest>
{
    ...
    /// </summary>
    public abstract override string ManifestPath { get; }

    /// <summary>
    /// Stage manifest loaded from JSON that defines scene order and transitions.
    /// Convenience property that returns <see cref="ManifestAwareNode{T}.LoadedData"/> as StageManifest.
    /// </summary>
    protected StageManifest? StageManifest => LoadedData;
    ...
    protected async Task AdvanceToNextSceneAsync()
    {
        ...
        if (_currentSceneId == null)
        {
            ...
            if (firstScene != null)
            {
                ...
                await ExecuteSceneAsync(firstScene).ConfigureAwait(false);
            }
            return;
        }
    ...
    public override async void _Ready()
    {
        ...
        try
        {
            // Load stage manifest that defines scene order and properties via IManifestAware
            LoadManifest();
            var stageManifest = LoadedData;
            if (stageManifest == null)
            {
                GD.PrintErr($"[StageController] Failed to load stage manifest from {ManifestPath}");
                return;
            }
