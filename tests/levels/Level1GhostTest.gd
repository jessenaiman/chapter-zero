...
func after_test() -> void:
	# Clean up runner after each test
	if _runner != null:
		_runner.dispose()
		_runner = null

func test_level_1_ghost_loads_successfully() -> void:
	# Load the level scene using scene_runner with proper cleanup
	_runner = auto_free(scene_runner("res://source/scenes/game_scene/levels/level_1_ghost/level_1_ghost.tscn"))

	# Verify scene loaded
	assert_that(_runner.scene()).is_not_null()
	assert_that(_runner.scene()).is_instanceof(Control)

	print("[Level1GhostTest] Level scene loaded successfully")

func test_ghost_dialogic_bridge_exists() -> void:
	# Load level scene with proper cleanup
	_runner = auto_free(scene_runner("res://source/scenes/game_scene/levels/level_1_ghost/level_1_ghost.tscn"))
